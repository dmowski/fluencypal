...
description: >-
  ...
  through now, read funnel/insights, sample first-quiz and first-conversation
  transcripts, pull unresolved Sentry issues for the same window, check
  INTERVENTIONS.md, and write the short "what's going today" report. Use when
  the user asks what happened today, what's going today, for today's/daily
  analytics or funnel report, to analyze the report, or similar. Do not wait
  for them to attach webApp/src/features/Analytics/Custom/AGENTS.md.
---

# Analyze analytics report

Read and follow `webApp/src/features/Analytics/Custom/AGENTS.md` immediately. That file is the source of truth for window math, export commands, first-speech transcript sampling, report template, event meanings, and Sentry. Do not invent a shorter process.
...
7. Reply in the short report template from the guide (include the Sentry bullet).
8. Update `LAST_REPORT.md` `Analyzed through` to the export `toIso`. Do not commit unless asked.
