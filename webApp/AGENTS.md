...
**Quiz → Just Talk:** Quiz creates an anonymous Firebase user as soon as the quiz loads (not at `goalReview`). Signed-in users skip that and keep their account. Everyone takes the same path: teacher selection (uid required) → `micPermission` (Allow microphone, then the browser prompt) → one “about you” clip on `before_recordAbout` → plan intro → `goalReview`. `goalReview` **Start Speaking** primes the mic in that click and navigates to `/practice?justTalk=open&autoStart=1`. Practice auto-starts that call unmuted only when the mic-prime session flag **or** consume-once `autoStart=1` is set; the first call uses mode `quiz-talk` as lesson 1 of their plan (closed question about the about-you clip; 5+ words → yes/no or A/B, not “tell me more”). At the free reply cap, the review offers the next plan card and still opens payment confirmation before Stripe. After that lesson, another `talk` / `quiz-talk` call stays locked until they have access, and `/practice?justTalk=open` shows that offer. Dashboard Just Talk stays mode `talk` until then. Cold `/practice?justTalk=open` before the lesson is used shows the Enable-mic panel (`just-talk-handoff`) until the user taps (call stays muted). `justTalk=open` stays until the first spoken user message. The call must start in **call** mode, not record. Mic/camera prompts may block automation — open `/practice?justTalk=open` without a mic prime to confirm the handoff panel still appears. Walking all of `/quiz` needs a voice clip and AI plan generation; use `justTalk=open` to verify the post-`goalReview` destination without finishing the quiz.
...
- Guide + **“What's going today?”** report format: `src/features/Analytics/Custom/AGENTS.md` (also the `analyze-analytics-report` skill; include Sentry and a sample of first-quiz / first-conversation transcripts for the same window)
