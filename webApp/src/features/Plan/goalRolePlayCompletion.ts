...
export const GOAL_ROLE_PLAY_EARLY_FINISH_USER_MESSAGES = 4;
...
export function countUserMessages(messages: { isBot: boolean }[]): number {
  return messages.filter((message) => !message.isBot).length;
}

export function isGoalRolePlayConversation(mode: ConversationType): boolean {
  return mode === 'goal-role-play';
}
...
export function canFinishGoalRolePlayLesson(input: {
  currentMode: ConversationType;
  userMessageCount: number;
  lessonProgress: number | null | undefined;
}): boolean {
  const progress = input.lessonProgress ?? 0;

  if (!isGoalRolePlayConversation(input.currentMode)) {
    return progress > 99;
  }
  ...
  return input.userMessageCount >= GOAL_ROLE_PLAY_EARLY_FINISH_USER_MESSAGES;
}
