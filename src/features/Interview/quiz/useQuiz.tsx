...
import { QuizSurvey2, QuizSurvey2FollowUpQuestion } from "@/features/Goal/Quiz/types";
...
`,
  ...
  const preFindNativeLanguage = async (
    langToLearn: string
  ): Promise<Partial<InterviewQuizUrlState>> => {
    try {
  ...
  const setLanguageToLearn = async (langToLearn: SupportedLanguage) => {
    const newPartialState: Partial<InterviewQuizUrlState> = {
      learn: langToLearn,
    };
    const updatedNativeLanguage = await preFindNativeLanguage(langToLearn);
    const newStatePatch: Partial<InterviewQuizUrlState> = {
      ...newPartialState,
...
export function InterviewQuizProvider({
  ...
  const hook = useProvideInterviewQuizContext({ pageLang, defaultLangToLearn, coreData, quiz });
