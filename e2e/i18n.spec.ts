...
test.describe("Internationalization", () => {
  test("should display Russian content with correct locale settings", async ({
    page,
  }) => {
    await page.goto("/ru");
    ...
    expect(
      content!.startsWith("Практикуйте разговорный английский с FluencyPal"),
    ).toBe(true);
  });

  test("should display French content with correct locale settings", async ({
    page,
  }) => {
    await page.goto("/fr");
    ...
    await expect(
      page.getByText("Apprenez n'importe où, n'importe quand"),
    ).toBeVisible();
  });

  test("should generate correct localized links in language switcher", async ({
    page,
  }) => {
    ...
    await expect(chineseLink).toHaveAttribute(
      "href",
      "/zh/scenarios/alias-game",
    );
  });

  test("should render correct meta tags for default locale", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    expect(content!.length).toBeGreaterThan(0);
    expect(
      content!.startsWith("Practice conversational English with FluencyPal"),
    ).toBe(true);
